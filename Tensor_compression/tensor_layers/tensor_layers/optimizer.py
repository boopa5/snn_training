...
from typing import List, Optional, Tuple, Union, Iterable, Any, Dict
...
class Adam_norm(Optimizer):
    ...
    def __setstate__(self, state):
        ...
        for group in self.param_groups:
            ...
            group.setdefault("normalize", False)
...
def adamw_norm(
    ...
    *,
    ...
    if fused and not torch.jit.is_scripting():
        ...
        func = _multi_tensor_adamw_norm
    else:
        func = _single_tensor_adamw_norm
    ...
    *,
    ...
    for i, param in enumerate(params):
        ...
        if capturable or differentiable:
            ...
            delta = exp_avg/denom
            if normalize:
                delta = -step_size*delta/(torch.norm(delta)+eps)*torch.norm(param)
            else:
    ...
    *,
    ...
    for ((
        ...
        if capturable:
            ...
            torch._foreach_div_(exp_avg_sq_sqrt, bias_correction1)


            torch._foreach_div_(exp_avg_sq_sqrt, device_exp_avgs) 
            torch._foreach_reciprocal_(exp_avg_sq_sqrt)  # The update before normalization
              

            #Start normalization
            if normalize:
                torch._foreach_div_(exp_avg_sq_sqrt, torch._foreach_norm(exp_avg_sq_sqrt))
                torch._foreach_mul_(exp_avg_sq_sqrt,torch._foreach_norm(device_params)) #normalized update

            # update params
            torch._foreach_mul_(exp_avg_sq_sqrt, step_size)
            torch._foreach_add_(device_params, exp_avg_sq_sqrt)
        else:
            bias_correction1 = [1 - beta1 ** _get_value(step) for step in device_state_steps]
            bias_correction2 = [1 - beta2 ** _get_value(step) for step in device_state_steps]

            step_size = _stack_if_compiling([-1*lr for bc in device_state_steps])

            bias_correction1 = _stack_if_compiling([1/bc for bc in bias_correction1])
